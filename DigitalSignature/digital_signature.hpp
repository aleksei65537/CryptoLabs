...
private:
    ...
    friend std::string sign<key_size, T>(const std::string&, RSA<key_size,T>&);

    friend bool verify<key_size, T>(const std::string&, const std::string&,
                                    RSA<key_size, T>&);

    std::string set_asn1_len(const std::string&) const;

    std::string set_asn1_int(const std::string&) const;
    ...
    size_t get_asn1_len(const std::string&) const;

    size_t get_asn1_int(const std::string&, T&);
...
public:
    ...
    RSA(const RSA& obj)
        ...
        , private_exponent(obj.private_exponent)
        , prime_1(obj.prime_1)
        , prime_2(obj.prime_2)
        , exponent_1(obj.exponent_1)
        , exponent_2(obj.exponent_2)
        , coefficient(obj.coefficient) { }

    RSA& operator=(const RSA& rhs)
    {
        modulus = rhs.modulus;
        public_exponent = rhs.public_exponent;
        private_exponent = rhs.public_exponent;
        prime_1 = rhs.prime_1;
        prime_2 = rhs.prime_2;
        exponent_1 = rhs.exponent_1;
        exponent_2 = rhs.exponent_2;
...
template<size_t key_size, typename T>
std::string RSA<key_size, T>::hex_str(const std::string& byte_data) const
{
    std::string s(byte_data.size() * 2, ' ');

    for (size_t i = 0; i < byte_data.size(); ++i)
    {
        s[2 * i]     = hex_map[(byte_data[i] & 0xF0) >> 4];
        s[2 * i + 1] = hex_map[byte_data[i] & 0x0F];
    }
...
template<size_t key_size, typename T>
size_t RSA<key_size, T>::get_asn1_len(const std::string& shex) const
{
...
template<size_t key_size, typename T>
size_t RSA<key_size, T>::get_asn1_int(const std::string& shex, T& big_int)
{
    size_t val_len;
    size_t len_len;
    size_t tag_len = 4;
    ...
    if (shex.substr(0, 2) == "02")
    {
        ...
        len_len *= 2;
        val_len = get_asn1_len(shex.substr(2, shex.size() - 2));
...
void RSA<key_size, T>::import_pubkey_from_hex(const std::string shex)
{
    ...
    {
        size_t seq_len = get_asn1_len(shex.substr(2, shex.size() - 2));
        size_t position = shex.size() - seq_len;
        position += get_asn1_int(
                shex.substr(position, shex.size() - position), modulus);
        get_asn1_int(shex.substr(position, shex.size() - position),
                                public_exponent);
    }
    else
    {
...
void RSA<key_size, T>::import_privkey_from_hex(const std::string shex)
{
    ...
    if(shex.substr(0, 2) == "30")
    {
        ...
        position += get_asn1_int(shex.substr(position, shex.size() - position),
                                 modulus);
        position += get_asn1_int(shex.substr(position, shex.size() - position),
                                 public_exponent);
        position += get_asn1_int(shex.substr(position, shex.size() - position),
...
template<size_t key_size, typename T>
void RSA<key_size, T>::import_privkey_from_pem(const std::string& pem)
{
    std::string tmp = pem;
...
template<size_t key_size, typename T>
void RSA<key_size, T>::import_pubkey_from_pem(const std::string& pem)
{
...
template<size_t key_size, typename T>
std::string RSA<key_size, T>::set_asn1_len(const std::string& shex) const
{
    std::stringstream ss;
...
template<size_t key_size, typename T>
std::string RSA<key_size, T>::set_asn1_int(const std::string& s) const
{
    std::string val = s;
    ...
    std::string len = set_asn1_len(val);
...
std::string RSA<key_size, T>::export_privkey_to_hex() const
{
    ...
    shex.insert(0, set_asn1_int(ss.str()));
    ss.str(std::string());
    ...
    ss << std::hex << modulus;
    shex.insert(0, set_asn1_int("00" + ss.str()));
    ss.str(std::string());

    shex.insert(0, "020100");

    shex.insert(0, set_asn1_len(shex));
    shex.insert(0, "30");
...
std::string RSA<key_size, T>::base64_to_hex(const std::string& s) const
{
    ...
    auto equal_count = std::count(base64_string.begin(), base64_string.end(),
                                  '=');
...
void RSA<key_size, T>::generate()
{
    ...
    coefficient < 0 ? coefficient += p : coefficient %= p;
}
...
T RSA<key_size, T>::encrypt(T m, bool use_pub_key) {
    return use_pub_key == true
        ...
        : boost::multiprecision::powm(m, this->private_exponent, this->modulus);
}
